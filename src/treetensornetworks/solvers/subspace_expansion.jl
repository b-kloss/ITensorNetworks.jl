...
function subspace_expansion_sweep!(
  ψ::ITensorNetworks.AbstractTTN{V}, PH::ITensorNetworks.AbstractProjTTN{V}; maxdim, cutoff, atol=1e-10, kwargs...
) where {V}

  order = ITensorNetworks.TDVPOrder(2, Base.Forward)
  direction = ITensorNetworks.directions(order)
