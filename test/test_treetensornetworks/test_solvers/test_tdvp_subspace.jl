...
@testset "MPS TDVP" begin
  ...
  @testset "exact vs 1s + local subspace" begin
    ...
    tdvp_kwargs = (
      ...
      updater_kwargs=(;
        expand_kwargs=(;
          cutoff=cutoff / dt, svd_func_expand=ITensorNetworks._svd_solve_normal
        ),
        exponentiate_kwargs=(;),
      ),
    ...
    @test inner(psi0, psi) ≈ 1 atol = 1e-12
    mag_exp = expect("N", psifer)
    @show maximum(abs.(real.([mag_exp[v] for v in vertices(psifer)]) .- res[:, end]))
    @test all(
      i -> i < 5e-3, abs.(real.([mag_exp[v] for v in vertices(psifer)]) .- res[:, end])
    )
  end
end
